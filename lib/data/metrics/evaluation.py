...
def rank(similarity, q_pids, g_pids, topk=[1, 5, 10], get_mAP=True):
    ...
    pred_labels = g_pids[indices.cpu()]  # q * k #BAO
...
def retrieve(similarity, g_pids, topk=[1, 5, 10]):
    max_rank = max(topk)
    _, indices = torch.topk(
        similarity, k=max_rank, dim=0, largest=True, sorted=True
    )  # q * topk
    
    pred_labels = g_pids[indices.cpu()]  # q * k
    
    pred_image_ids = list(dict.fromkeys(pred_labels))
...
def retrieve_results(
    ...
    logger = logging.getLogger("PersonSearch.inference")
    data_dir = os.path.join(output_folder, "database_features.npz")
    
    if predictions is None:
        inference_data = np.load(data_dir)
        logger.info("Load inference data from {}".format(data_dir))
        image_ids = torch.tensor(inference_data["image_ids"])
        image_global = torch.tensor(inference_data["image_global"])
        text_global = torch.tensor(inference_data["text_global"])
    else: 
        image_ids, pids = [], []
        image_global, text_global = [], []
        ...
        if save_data:
            np.savez(
                ...
                image_global=image_global.cpu().numpy(),
                text_global=text_global.cpu().numpy()
            )

    image_sample, text_sample = sample_prediction
    image_sample = F.normalize(image_sample)
    text_sample = F.normalize(text_sample)
    
    image_similarity = torch.matmul(image_global.cuda(), image_sample.T)
    text_similarity = torch.matmul(text_global.cuda(), text_sample.T)
    total_similarity = (image_similarity + text_similarity)/2
        
    return retrieve(total_similarity, torch.tensor(image_ids), topk)
